...
export default function CongestionHeatmap({ zones }: CongestionHeatmapProps) {
  ...
  // Sample data for the line chart - formatted for Recharts
  ...
  const maxCrowd = Math.max(...crowdHistoryData.map(item => item.people));
  const minCrowd = Math.min(...crowdHistoryData.map(item => item.people));
  const averageCrowd = Math.round(crowdHistoryData.reduce((sum, item) => sum + item.people, 0) / crowdHistoryData.length);
      ...
      <div className="border-t pt-6">
        ...
        <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-100">
          ...
          <div className="grid grid-cols-3 gap-4 text-center">
            ...
            <div>
              <div className="text-2xl font-bold text-gray-900">{averageCrowd}</div>
              <div className="text-sm text-gray-500">Average</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-gray-900">{minCrowd}</div>
